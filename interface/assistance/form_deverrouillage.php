         ...
         foreach($_SESSION["liste_formations"] as $formation_array)
         {
            ...
            $corps_message.="<tr>
                              <td class='td-gauche' width='10px'>
                                 <input type='hidden' name='spec_nom[".htmlspecialchars(stripslashes($formation_array["cand_id"]))."]' value='".htmlspecialchars(stripslashes($formation_array["nom"]), ENT_QUOTES)."'>
                                 $selection
                              </td>
...
<div class='main'>
   ...
   <tr>
      ...
      <td class='td-droite fond_menu'>
         <textarea name='motif' cols="50" rows="7"><?php if(isset($motif)) echo htmlspecialchars(stripslashes($motif), ENT_QUOTES); ?></textarea>
      </td>
   </tr>
   </table>
