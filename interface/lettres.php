...
if(isset($_GET["p"]) && -1!=($params=get_params($_GET["p"])))
{
   ...
   {
      $res_cand=db_query($dbr, "SELECT $_DBC_cand_decision, $_DBC_cand_propspec_id FROM $_DB_cand 
                                 WHERE $_DBC_cand_id='$params[cand_id]' 
                                 AND $_DBC_cand_candidat_id='$_SESSION[authentifie]'
                                 AND $_DBC_cand_statut='1'");
                                 
      if(db_num_rows($res_cand))
      {
         list($decision_id, $propspec_id)=db_fetch_row($res_cand, 0);
...
else
{
   ...
   $ensemble_candidats=array($_SESSION["authentifie"] => $cand_id);
}
            ...
            if(!empty($candidature_array["spec_nom"]))
            {
               ...
               if($candidature_array["groupe_spec"]!="-1")
               {
                  ...
                  if(count($candidatures_multiples) > 1) // si =1 : lettre normale
                  {
                     ...
                     {
                        if($cand_m_array["decision"]==$__DOSSIER_ADMIS_AVANT_CONFIRMATION || $cand_m_array["decision"]==$__DOSSIER_ADMIS || $cand_m_array["decision"]==$__DOSSIER_ADMISSION_CONFIRMEE ||$cand_m_array["decision"]==$__DOSSIER_ADMIS_LISTE_COMP || $cand_m_array["decision"]==$__DOSSIER_SOUS_RESERVE)
                           $admission_trouvee=1;
...
*/
                     ...
                     if($flag_adr_cand=="t")
                     {
                        ...
                        $candidat_adresse=civ_lang($candidat_array["civilite"], $lettre_lang, 2). " " .  $candidat_array["nom"] . " " . $candidat_array["prenom"] . "\n" . $candidat_array["adresse"];
...
*/
      ...
      write_evt($dbr, $__EVT_ID_C_PREC, "Lettre : $candidature_array[nom_complet] : $candidature_array[decision_txt]", $candidat_id, $cand_id);
   }
   ...
   if(isset($nb_pages) && $nb_pages>0)
   {
      ...
      else
      {
         ...
         $nom_fichier=clean_str($_SESSION["authentifie"] . "_$date.pdf");
      }
