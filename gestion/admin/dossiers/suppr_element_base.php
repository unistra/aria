...
<?php
  ...
  {
    $_SESSION["suppr_element_id"]=$_POST["element_id"];
    $resultat=1;
  }

  if(isset($_POST["go_valider"]) || isset($_POST["go_valider_x"]))
  {
    db_query($dbr,"DELETE FROM $_DB_dossiers_elems WHERE $_DBC_dossiers_elems_id='$_SESSION[suppr_element_id]'");
...
<div class='main'>
  <?php
    titre_page_icone("Supprimer un élément de la base de données", "trashcan_full_32x32_slick_fond.png", 15, "L");

    print("<form method='post' action='$php_self'>\n");

    if(isset($_GET["succes"]))
      message("Elément supprimé avec succès de la base de données", $__SUCCES);

    if(!isset($resultat))
    {
      message("<center>Sécurité : seuls les éléments NON RATTACHES sont supprimables.
  ...
  <tr>
    <td class='td-gauche fond_menu2'>
      <font class='Texte_menu2'><b>Elément à supprimer de la base de données :</b></font>
    </td>
    <td class='td-droite fond_menu'>
      <?php
        $result=db_query($dbr,"SELECT $_DBC_dossiers_elems_id, $_DBC_dossiers_elems_intitule
                          FROM $_DB_dossiers_elems
                        WHERE $_DBC_dossiers_elems_id NOT IN (SELECT $_DBC_dossiers_ef_elem_id FROM $_DB_dossiers_ef)
                        AND $_DBC_dossiers_elems_comp_id='$_SESSION[comp_id]'
                          ORDER BY $_DBC_dossiers_elems_intitule");
        ...
        {
          $no_element=1;
          print("<font class='Texte_important_menu'>Aucun élément supprimable</font>");
        }
