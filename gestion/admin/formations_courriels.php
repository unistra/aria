...
/*													
		&& db_num_rows(db_query($dbr, "SELECT * FROM $_DB_acces WHERE $_DBC_acces_id='$_GET[aid]'
	...
	<?php
		...
		if(!isset($affectation))	// Affichage de toutes les affectations
		{
         ...
         if($rows)
         {
            for($i=0; $i<$rows; $i++)
            {
               list($a_id, $a_nom, $a_prenom)=db_fetch_row($result, $i);
            
               if($a_id!="0")
	  				  print("<tr>
	  					 		 <td class='fond_menu' style='padding:2px 5px 2px 5px;'>
	  								 <font class='Texte_menu'>- $a_nom $a_prenom</font>
								 </td>
         ...
         db_free_result($result);
			...
			if($rows)
			{
				for($i=0; $i<$rows; $i++)
				{
					list($propspec_id, $annee_id, $annee, $annee_ordre, $spec_nom, $finalite, $mention, $mention_nom, $manuelle, $a_id, $a_nom, $a_prenom)=db_fetch_row($result, $i);
